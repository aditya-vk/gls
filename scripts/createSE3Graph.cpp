...
{
  Eigen::Quaternionf q(haltonPoint(6), haltonPoint(3), haltonPoint(4), haltonPoint(5));
  auto euler = q.toRotationMatrix().eulerAngles(0, 1, 2);

  // Convert positions to Eigen. DART To OMPL settings: angles, positions: ax, az, ay, x, z, y.
  Eigen::VectorXd positions(6);
  positions << euler(0), euler(2), euler(1), haltonPoint(0), haltonPoint(2), haltonPoint(1);
    ...
    Eigen::VectorXd upperLimits)
{
  ...
  pos << scaledConfig(0), scaledConfig(1), scaledConfig(2);
  quat << scaledConfig(3), scaledConfig(4), scaledConfig(5), scaledConfig(6);
  double quat_norm = quat.norm();
  quat = quat/quat_norm;

  Eigen::VectorXd finalConfig(7);
  finalConfig << pos(0), pos(1), pos(2), quat(0), quat(1), quat(2), quat(3);
                ...
                Eigen::VectorXd lowerLimits, Eigen::VectorXd upperLimits, double threshold, bool knn)
{
  ...
  std::string vertexFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/vertices_se3.txt";

  // Holds the edge information. <source vertex ID> <target vertex ID> <length>
  std::string edgesFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/edges_se3.txt";

  // Holds each edge's source and target configurations. Useful to visualize the edges.
  // Not necessary for graph generation.
  std::string edgesVizFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/edges_viz_se3.txt";
  ...
  Eigen::VectorXd offset = Eigen::VectorXd::Random(7)*0;
  ...
  for(const auto& config : configurations)
  {
    ...
    {
      vertexLogFile << config[idx] << " ";
    }
    vertexLogFile << std::endl;
  }
  ...
  bounds.setLow(0, -300.0);
  bounds.setLow(1, -300.0);
  bounds.setLow(2, -300.0);

  bounds.setHigh(0, 400.0);
  bounds.setHigh(1, 400.0);
  bounds.setHigh(2, 400.0);
  ...
  if (knn)
  {
    for (std::size_t i = 0; i < configurations.size()-1; ++i)
    {
      ...
      for (std::size_t j = i+1; j < configurations.size(); ++j)
      {
        ...
        ompl::base::ScopedState<ompl::base::SE3StateSpace> end(si);

        start->setX(configurations[i](0));
        start->setY(configurations[i](1));
        start->setZ(configurations[i](2));
        start->rotation().x = configurations[i](3);
        start->rotation().y = configurations[i](4);
        start->rotation().z = configurations[i](5);
        start->rotation().w = configurations[i](6);

        end->setX(configurations[j](0));
        end->setY(configurations[j](1));
        end->setZ(configurations[j](2));
        end->rotation().x = configurations[j](3);
        end->rotation().y = configurations[j](4);
        end->rotation().z = configurations[j](5);
        end->rotation().w = configurations[j](6);

        // std::cout << configurations[i].transpose() << std::endl;
        // std::vector<double> positions_ompl;
        // space->copyToReals(positions_ompl, start.get());
        // for (int id = 0; id < positions_ompl.size(); ++id)
          // std::cout << positions_ompl[id] << " ";
        // std::cout << std::endl;

        auto distance = space->distance(start.get(), end.get());
...
int main(int argc, char *argv[])
{
  ...
  Eigen::VectorXd lowerLimits(7);
  Eigen::VectorXd upperLimits(7);
  lowerLimits << -73, -179, 0.0, -1, -1, -1, -1;
  upperLimits << 300, 168, 90.0, 1, 1, 1, 1;

  Eigen::VectorXd difference(7);
  difference = upperLimits - lowerLimits;
  double threshold = std::pow(numSamples, -1.0/6)*std::max(difference(0), difference(1));
