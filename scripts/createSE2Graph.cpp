                ...
                Eigen::VectorXd lowerLimits, Eigen::VectorXd upperLimits, double threshold, bool knn, Eigen::VectorXd offset, int idx)
{
  ...
  std::string vertexFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/vertices_" + std::to_string(numSamples) + ".txt";

  // Holds the edge information. <source vertex ID> <target vertex ID> <length>
  std::string edgesFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/edges_" + std::to_string(numSamples) + ".txt";

  // Holds each edge's source and target configurations. Useful to visualize the edges.
  // Not necessary for graph generation.
  std::string edgesVizFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/edges_viz_" + std::to_string(numSamples) + ".txt";
...
int main(int argc, char *argv[])
{
  ...
  {
    auto offset = Eigen::VectorXd::Random(3)*0.0;
...
//   // Holds the vertices. Index is the line number. Content is the configuration.
//   std::string vertexFile = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/free_vertices1.txt";
