...
void shortcutAndLog(ompl::base::ProblemDefinitionPtr pdef, double plantime, double timelimit, double costlimit)
{
  ...
  std::string filename = "/home/adityavk/workspaces/lab-ws/src/planning_dataset/results/rrt_reach_for_shelf_constrained.txt";
...
// =======================
  ...
  rightFarLeftReach << 5.30, 1.50, -0.12, 0.87, 1.21, 0.34, 0.13;
...
// =======================
  ...
  SkeletonPtr pitcher2;
  Eigen::Isometry3d pitcherPose2;
  SkeletonPtr roof;
  Eigen::Isometry3d roofPose;
  SkeletonPtr countertop;
  Eigen::Isometry3d counterPose;
  ...
  // Poses for Pitcher
  pitcherPose2 = Eigen::Isometry3d::Identity();
  pitcherPose2.translation() = tablePose.translation() + 
    Eigen::Vector3d(0.0, 0.3, 0.73);
  ...
  // Poses for countertop
  counterPose = Eigen::Isometry3d::Identity();
  counterPose.translation() = Eigen::Vector3d(-0.5, -1.2, 0);
  rot = Eigen::AngleAxisd(0, Eigen::Vector3d::UnitZ())
        * Eigen::AngleAxisd(0, Eigen::Vector3d::UnitY())
        * Eigen::AngleAxisd(0, Eigen::Vector3d::UnitX());
  counterPose.linear() = rot;
  ...
  pitcher2 = makeBodyFromURDF(resourceRetriever, pitcherURDFUri, pitcherPose2);
  roof = makeBodyFromURDF(resourceRetriever, roofURDFUri, roofPose);
  countertop = makeBodyFromURDF(resourceRetriever, tableURDFUri, counterPose);
